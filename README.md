# coffeeman Guess The Flag game

A fast-paced and beautiful web app to test your global flag knowledge. Built with Vanilla HTML, CSS, JavaScript, and the REST Countries API.

🚀 Features
✅ Real-time data for 250+ countries via API

✅ 10-second countdown timer with visual color alerts

✅ Persistent High Score tracking

✅ Smooth animations and mobile-responsive layout

✅ Stored locally in your browser (no account needed)

📱 How to Use
Observe the flag displayed in the center card.

Select the correct country name from the four options below.

Act fast — you only have 10 seconds before the round ends!

The timer bar turns red when you have less than 3 seconds left.

If you choose correctly, your score increases. If you miss, the correct answer is highlighted.

Click "Continue" to move to the next flag and beat your high score.

🔐 Data Storage & Privacy
Your High Score is saved locally on your device using Local Storage.

This means your progress stays private and is not uploaded to any server.

If you clear your browser cache, your high score will be reset.

No personal data or location tracking is used.

🌐 The app link
👉 Click here to use the app

📲 Add to Home Screen (For Easy Access) You can install this quiz like a native app on your phone! Here's how:

✅ For iPhone (iOS Safari):

Open this app in Safari.

Tap the Share button (the square with the arrow).

Scroll down and tap Add to Home Screen.

Tap Add in the top-right corner.

✅ For Android (Chrome or Edge):

Open this app in Google Chrome or Microsoft Edge.

Tap the three-dot menu in the top-right corner.

Tap Add to Home screen.

Confirm by tapping Add.

🛠️ Tech Stack
HTML5 & CSS3 (Modern Flexbox/Grid)

Vanilla JavaScript (ES6+)

REST Countries API for data

LocalStorage for score persistence

# CoffeeMan Guess The Flag 🚩 全球国旗挑战小游戏
一个极简且美观的网页应用，带你挑战全球国旗识别。 基于 HTML、CSS、JavaScript 开发，实时调用全球国家数据。

🚀 功能特点
✅ 实时获取 250+ 国家和地区的国旗数据

✅ 10 秒倒计时，最后 3 秒自动变红提醒

✅ 自动记录个人最高得分（High Score）

✅ 适配所有手机屏和电脑端

✅ 数据保存在你的浏览器本地（无需注册账号）

📱 使用方法
观察中心卡片显示的国旗

从下方的四个选项中选择正确的国家名称

快速反应 —— 每一轮只有 10 秒钟时间！

计时条在最后 3 秒会变成红色以示预警

答对得分，答错则会为你亮出正确答案以供学习

点击 "Continue" 继续下一关，不断刷新你的最高纪录

🔐 数据存储与隐私
你的最高纪录保存在你设备的本地存储（Local Storage）

你的数据不会上传到任何地方，完全私密

如果你清空浏览器缓存或换设备，纪录会丢失

🛠️ 技术栈
HTML, CSS, JavaScript

REST Countries API（国旗数据源）

LocalStorage（离线高分存储）

🧔 Author
Created by @CoffeeManApp

Donation: @Buymeacoffee
